...
import com.gu.mediaservice.lib.auth.provider.{Authenticated, AuthenticationProviderResources, AuthenticationStatus, NotAuthenticated, UserAuthenticationProvider}
...
class BBCPPProxyAuthenticationProvider (resources: AuthenticationProviderResources, providerConfiguration: Configuration)
  ...
  private def gridUserFrom(request: RequestHeader, bbcUser: BBCBasicUserInfo): UserPrincipal = {
    ...
    val userGroups: TypedEntry[List[UserGroup]] = TypedEntry[List[UserGroup]](BBCPPProxyAuthenticationProvider.userGroupKey, bbcUser.userGroups)
