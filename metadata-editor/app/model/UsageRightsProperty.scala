...
import com.gu.mediaservice.lib.config.{Photographers, Publication, UsageRightsConfig}
...
object UsageRightsProperty {
  ...
  type AllowedUsageRights = List[String]
  val props: List[(UsageRightsSpec, Photographers, AllowedUsageRights) => List[UsageRightsProperty]] =
  ...
  def getPropertiesForSpec(u: UsageRightsSpec, p: Photographers, usageRights: AllowedUsageRights): List[UsageRightsProperty] = props.flatMap(f => f(u, p, usageRights))
  ...
  private def restrictionProperties(u: UsageRightsSpec, p: Photographers, usageRights: AllowedUsageRights): List[UsageRightsProperty] = u match {
    case NoRights => List()
    case _ => List(UsageRightsProperty("restrictions", "Restrictions", "text", u.defaultCost.contains(Conditional)))
  }

  def categoryUsageRightsProperties(u: UsageRightsSpec, p: Photographers, usageRights: AllowedUsageRights) = u match {
    case Agency => List(
      requiredStringField("supplier", "Supplier", Some(sortList(UsageRightsConfig.freeSuppliers))),
      UsageRightsProperty(
