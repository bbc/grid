...
class QueryBuilderTest extends FunSpec with Matchers with ConditionFixtures with Fixtures {
  ...
  describe("get advanced file metadata") {
    it("should return the elasticsearchPath or the field parameter itself if the config doesn't exist"){
      queryBuilder.getAdvancedFieldPath("bbcElvisCollection") shouldBe "bbcElvisCollection"

      queryBuilder.getAdvancedFieldPath("credit") shouldBe "metadata.credit"
    }
  }
