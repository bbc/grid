...
class ImgProxyUrlBuilderTest extends AnyFunSuiteLike {
  ...
  test("fixedUri uses imgproxy's insecure signature when no signing is provided") {
    val uri = ImgProxyUrlBuilder.fixedUri(baseUri, sourceUri, width = 100, height = 200, quality = 80)
  ...
  test("fixedUri signs the request with imgproxy's standard HMAC-SHA256 scheme when signing is provided") {
    ...
    val uri = ImgProxyUrlBuilder.fixedUri(baseUri, sourceUri, width = 100, height = 200, quality = 80, signing = Some(signing))
  ...
  test("fixedUri includes a non-zero rotation") {
    val uri = ImgProxyUrlBuilder.fixedUri(baseUri, sourceUri, width = 100, height = 200, quality = 80, rotationDegrees = -90)
